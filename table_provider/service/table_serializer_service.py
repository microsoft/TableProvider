...
from table_provider.model.table_provider_enum import TableSerializationType
...
class TableDataSerializer:
    ...
    def __init__(self):
        ...
        self.use_structure_mark = False
        self.add_grammar = False
    ...
    @staticmethod
    def _generate_additional_knowledge(structured_data_dict, use_structure_mark):
        if use_structure_mark:
            additional_knowledge = (
                    "<title>\n"
                    + "".join(structured_data_dict["title"])
                    + "\n"
                    + "<context>\n"
                    + "".join(structured_data_dict["context"])
                    + "\n"
                    + "<caption>\n"
                    + "".join(structured_data_dict["table"]["caption"])
                    + "\n"
            )
        else:
            additional_knowledge = (
    ...
    def linearize_markdown_grid(self):
        ...
        """
        additional_knowledge = self._generate_additional_knowledge(self.structured_data_dict, self.use_structure_mark)
        if self.change_order:
            structured_data = pd.DataFrame(
                np.array(self.structured_data_dict["table"]["rows"]).T,
